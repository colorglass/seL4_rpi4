...
void GEC_FN(gec_mk_privkey)(struct gec_privkey *k,
                            const uint8_t privkey[GEC_PRIV_KEY_LEN],
                            const uint8_t pubkey[GEC_PUB_KEY_LEN]) {
  memcpy(k->pub, pubkey, GEC_PUB_KEY_LEN);
  memcpy(k->priv, privkey, GEC_PRIV_KEY_LEN);
}

void GEC_FN(gec_mk_pubkey)(struct gec_pubkey *k,
                           const uint8_t pubkey[GEC_PUB_KEY_LEN]) {
  memcpy(k->pub, pubkey, GEC_PUB_KEY_LEN);
}

void GEC_FN(gec_init_sym_key_conf_auth)(struct gec_sym_key *k,
                                        const uint8_t rawkey[GEC_RAW_KEY_LEN]) {
...
void GEC_FN(gec_key_material_to_2_channels)(
    struct gec_sym_key *chan1, struct gec_sym_key *chan2,
...
void GEC_FN(gec_init_sym_key_conf)(struct gec_sym_key_conf *k,
                                   const uint8_t rawkey[GEC_RAW_KEY_LEN]) {
  const uint8_t *p = rawkey;
...
// Authenticated encryption mode over statically sized messages.  This is often
// termed "crypto_box()".
//
...
int GEC_FN(gec_encrypt)(struct gec_sym_key *k, const uint8_t pt[GEC_PT_LEN],
  ...
  if (UINT32_MAX - nrBlks <= k->ctr) {
    ...
    gcm_ret = gcm_encrypt_message(iv, _GCM_IV_LEN, NULL, 0,
                                  ct + _GEC_ID_LEN + _GEC_CTR_LEN, GEC_PT_LEN,
                                  tag, _GEC_TAG_LEN, &k->gctx);
...
int GEC_FN(gec_decrypt)(struct gec_sym_key *k, const uint8_t ct[GEC_CT_LEN],
  ...
  // } else {
    ...
    gcm_ret = gcm_decrypt_message(iv, _GCM_IV_LEN, NULL, 0 // AAD
                                  ,
                                  pt, GEC_PT_LEN, tag, _GEC_TAG_LEN, &k->gctx);
    memset(iv, 0, _GCM_IV_LEN);
...
// Confidentiallity-only encryption and decryption mode over statically sized
// messages.
int GEC_FN(gec_encrypt_conf)(struct gec_sym_key_conf *k, const uint8_t *pt,
...
int GEC_FN(gec_decrypt_conf)(struct gec_sym_key_conf *k, const uint8_t *ct,
...
void GEC_FN(gec_generate_sign_keypair)(struct gec_privkey *q,
                                       struct gec_pubkey *p) {
...
void GEC_FN(gec_sign)(const struct gec_privkey *k, const uint8_t *msg,
                      const size_t msg_len, uint8_t sig[GEC_SIG_LEN]) {
  ed25519_sign(msg, msg_len, k->priv, k->pub, sig);
}

// Given a public key, message and tag (signature) return 0 if the signature is
// correct, non-zero otherwize
int GEC_FN(gec_verify)(const struct gec_pubkey *k, const uint8_t *msg,
                       const size_t msg_len, const uint8_t sig[GEC_SIG_LEN]) {
...
void GEC_FN(gec_generate_ephemeral_keypair)(
    const uint8_t gec_ephemeral_priv[GEC_PRIV_EPHEMERAL_KEY_LEN],
    uint8_t gec_ephemeral_pub[GEC_PUB_EPHEMERAL_KEY_LEN]) {
...
void GEC_FN(gec_ecdh)(uint8_t secret_bytes[GEC_SECRET_BYTES_LEN],
                      const uint8_t pub_asym[GEC_PUB_EPHEMERAL_KEY_LEN],
                      const uint8_t priv_asym[GEC_PRIV_EPHEMERAL_KEY_LEN]) {
